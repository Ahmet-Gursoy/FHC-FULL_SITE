...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
        ...
        $foto = 'uploads/calisan_' . time() . '.' . $ext;
    ...
    if ($id > 0) {
        // Güncelleme
        if ($foto) {
            $conn->query("UPDATE calisanlar SET ad='$ad', soyad='$soyad', pozisyon='$pozisyon', departman='$departman', telefon='$telefon', email='$email', foto='$foto' WHERE id=$id");
        } else {
            $conn->query("UPDATE calisanlar SET ad='$ad', soyad='$soyad', pozisyon='$pozisyon', departman='$departman', telefon='$telefon', email='$email' WHERE id=$id");
        }
        $alert = '<div class="alert alert-success">Çalışan bilgileri güncellendi.</div>';
    } else {
        // Ekleme
        if ($foto) {
            $conn->query("INSERT INTO calisanlar (ad, soyad, pozisyon, departman, telefon, email, foto) VALUES ('$ad', '$soyad', '$pozisyon', '$departman', '$telefon', '$email', '$foto')");
        } else {
            $conn->query("INSERT INTO calisanlar (ad, soyad, pozisyon, departman, telefon, email) VALUES ('$ad', '$soyad', '$pozisyon', '$departman', '$telefon', '$email')");
        }
        $alert = '<div class="alert alert-success">Yeni çalışan eklendi.</div>';
    }
}

// Çalışanları departmanlara göre çek
...
// Düzenleme için çalışan bilgilerini çek
$edit_calisan = null;
...
$departman_isimleri = [
    ...
    'aile_sagligi_calisanlari' => 'Aile Sağlığı Çalışanlarımız',
